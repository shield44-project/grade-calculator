...
export default function CourseCard({ id, onUpdate, initialCourseData }) {
  ...
  return (
    <div className="group relative overflow-hidden rounded-2xl transition-all duration-300 hover:scale-[1.01] animate-slideInLeft">
      ...
      <div className="absolute inset-0 bg-gradient-to-r from-cyan-500/30 to-orange-500/30 rounded-2xl blur-md opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
          ...
          <div className="flex justify-between items-start">
            <div className="flex-grow">
              ...
              <div className="flex gap-3 flex-wrap">
                <span className="px-4 py-2 rounded-xl glass-effect text-xs font-bold text-cyan-300 inline-flex items-center gap-1">
      ...
      <>
          ...
          <div className="p-6 mt-4 glass-effect rounded-xl border border-cyan-500/20">
          ...
          <div className="mt-6 relative overflow-hidden rounded-xl">
            ...
            <div className="relative neumorphic-inset p-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                ...
                <ResultDisplay label="Points" value={results.points} color="text-orange-400" />
              </div>
            </div>
          </div>
        </>
...
const InputField = ({ name, label, placeholder, value, onChange }) => (
    <div className="group">
        <label className="block text-sm font-semibold text-gray-400 mb-2 group-focus-within:text-cyan-400 transition-colors duration-200">{label}</label>
        <input 
            type="number" 
            name={name} 
            ...
            value={value || ''} 
            onChange={onChange} 
            className="w-full p-3 neumorphic-inset rounded-xl text-white placeholder-gray-600 focus:ring-2 focus:ring-cyan-500 transition-all duration-300" 
        />
    </div>
);

const ResultDisplay = ({ label, value, color }) => (
    <div className="text-center p-4 rounded-xl glass-effect hover:shadow-[0_0_20px_rgba(0,212,255,0.2)] transition-all duration-300 transform hover:scale-105">
...
const RequiredSEEDisplay = ({ totalCie, cieMax, seeMax, hasSeeMarks }) => {
    ...
    return (
        <div className="p-6 mt-4 relative overflow-hidden rounded-xl glass-effect border border-cyan-500/20">

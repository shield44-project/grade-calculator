    ...
    <>
      {isLoggedIn ? (
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-gradient-to-r from-pink-500/20 to-teal-500/20 border border-pink-500/30">
            <svg className="w-5 h-5 text-pink-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
            </svg>
            <span className="text-sm font-medium text-pink-300">{username}</span>
          </div>
          <button
            onClick={handleLogout}
            className="px-4 py-2 text-sm font-medium text-gray-400 hover:text-pink-400 transition-colors duration-200"
          >
            Logout
          </button>
        </div>
        ...
        <button
          ...
          className="px-6 py-2.5 rounded-full bg-gradient-to-r from-pink-500 to-teal-500 hover:from-pink-600 hover:to-teal-600 text-white font-semibold text-sm transition-all duration-200 transform hover:scale-105 shadow-lg hover:shadow-xl border border-pink-400/30"
        >
          Login
        </button>
      )}
        ...
        <div className="fixed inset-0 z-[9999] flex items-start justify-center p-4 bg-black/70 backdrop-blur-sm animate-fadeIn overflow-y-auto" onClick={(e) => {
          ...
          <div className="relative bg-gray-900 rounded-xl shadow-2xl border border-gray-800 p-5 max-w-sm w-full animate-slideUp mt-12 sm:mt-20 mb-8" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-pink-400 to-teal-400">
                Login
              </h2>
              <button
            ...
            <form onSubmit={handleLogin} className="space-y-3">
              ...
              <div className="flex gap-2 pt-3">
                <button
                  type="submit"
                  className="flex-1 px-4 py-2 text-sm rounded-lg bg-gradient-to-r from-pink-500 to-teal-500 hover:from-pink-600 hover:to-teal-600 text-white font-medium transition-all duration-200 transform hover:scale-105"
                >
                  Login
                </button>
                <button

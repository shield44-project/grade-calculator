...
export const calculateRequiredSEEIntegrated = (cieTheory, cieLab, cieBreakdown, seeBreakdown, cieMax, seeMax) => {
  // Validate inputs
  if (!cieTheory && !cieLab) return null;
  ...
  gradingSystem.forEach(grade => {
    if (grade.grade !== 'F') {
      ...
      if (requiredTotalSEE <= seeMax && requiredTotalSEE >= minTotalSEE) {
        ...
        const requiredSEETheory = Math.max(minSEETheory, requiredTotalSEE * theoryRatio);
        const requiredSEELab = Math.max(minSEELab, requiredTotalSEE * labRatio);
        
        // Adjust if the sum exceeds required total (due to minimum constraints)
        let adjustedTheory = requiredSEETheory;
        let adjustedLab = requiredSEELab;
        
        if (requiredSEETheory + requiredSEELab > requiredTotalSEE) {
          // If minimums force us above required, just use minimums
          adjustedTheory = minSEETheory;
          adjustedLab = minSEELab;
        }
        
        results[grade.grade] = {
          required: Math.ceil(requiredTotalSEE),
          requiredTheory: Math.ceil(adjustedTheory),
          requiredLab: Math.ceil(adjustedLab),

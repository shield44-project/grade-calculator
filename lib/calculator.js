...
// 1 quiz (5), tests (25), EL 20 = 50 CIE
export const calculateKannadaCIE = (cieMarks) => {
    const { quiz, test1, test2, expLearning } = cieMarks;
    
    const quizValue = Number(quiz) || 0; // Out of 5
    
    // 2 tests, sum should equal 25 marks total
    const test1Value = Number(test1) || 0;
    const test2Value = Number(test2) || 0;
    const testTotal = test1Value + test2Value; // Sum of both tests (total: 25 marks)
    
    const expLearningValue = Number(expLearning) || 0; // Out of 20
    
    return Math.min(50, quizValue + testTotal + expLearningValue);
};

...
function analyzeRequiredSEEForGrade(cieMarks, cieMax, seeMax, targetGradePoints, courseDetails) {
  ...
  let effortLevel = 0;
  if (seePercentageNeeded < 35) effortLevel = 5; // Already achievable with minimum
  else if (seePercentageNeeded < 45) effortLevel = 15; // Very easy
  else if (seePercentageNeeded < 55) effortLevel = 30; // Easy
  else if (seePercentageNeeded < 65) effortLevel = 45; // Moderate-easy
  else if (seePercentageNeeded < 75) effortLevel = 60; // Moderate
  else if (seePercentageNeeded < 85) effortLevel = 75; // Hard
  else if (seePercentageNeeded < 95) effortLevel = 88; // Very hard
  ...
  return {
    ...
    difficulty: seePercentageNeeded > 100 ? 'Impossible' : 
                seePercentageNeeded > 95 ? 'Extremely Hard' :
                seePercentageNeeded > 85 ? 'Very Hard' :
                seePercentageNeeded > 75 ? 'Hard' :
                seePercentageNeeded > 65 ? 'Moderate' :
                seePercentageNeeded > 55 ? 'Moderate-Easy' :
                seePercentageNeeded > 45 ? 'Easy' :
                seePercentageNeeded > 35 ? 'Very Easy' : 'Already Secured',
    effortLevel
  };
}
